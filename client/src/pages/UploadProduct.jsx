...
import uploadImageProduct from "../utils/UploadImageProduct";
...
const UploadProduct = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setIsLoading(true)
      let uploadedImageUrls = [...data.image]; // เริ่มต้นด้วยรูปภาพที่มีอยู่แล้ว
      // console.log("🔄 Before Uploading: ", uploadedImageUrls);
  
      if (selectedFile && selectedFile.length > 0) {
        // อัปโหลดไฟล์ทั้งหมดไปที่ Cloud
        const resUpload = await uploadImageProduct(selectedFile);
  
        // console.log("📤 Upload Response:", resUpload);
  
        if (resUpload?.data?.data && resUpload.data.data.length > 0) {
          // ดึง URL ของแต่ละภาพที่อัปโหลดสำเร็จ
          const newUploadedUrls = resUpload.data.data.map((image) => image.url);
          uploadedImageUrls = [...uploadedImageUrls, ...newUploadedUrls];
          // console.log("✅ Uploaded Image URLs:", uploadedImageUrls);
        } else {
          console.error("❌ Upload failed: No image URL returned");
          return AxiosToastError("Upload image failed.");
        }
      }
  
      // สร้าง payload พร้อมอัปเดต image เป็น array
      const payload = {
        ...data,
        image: uploadedImageUrls, // ส่งรูปภาพทั้งหมด
        category: data.category.map((cat) => ({
          id: cat.id,
          name: cat.name,
        })),
        subCategory: data.subCategory.map((subCat) => ({
          id: subCat.id,
          name: subCat.name,
        })),
      };
      ...
      if (responseData?.success) {
        ...
        setPreviewImage([]); // เคลียร์ตัวอย่างรูป
        setSelectedFile([]); // เคลียร์ไฟล์ที่เลือก
      } else {
