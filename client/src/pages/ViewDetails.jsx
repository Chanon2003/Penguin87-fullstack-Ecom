...
const ViewDetails = ({ data, close }) => {
  // console.log('viewdetialdata', data)
  return (
          ...
          <div className="space-y-4">
            {data?.productOnOrder?.map((item) => (
              <div key={item.product.id} className="flex items-center gap-4 p-3 border rounded-md">
                <img
                  src={item.product.image?.[0]?.url || "/fallback-image.jpg"}
                  alt={item.product.name}
                  className="w-16 h-16 object-cover rounded-md border"
                />
                <div className="flex-1">
                  <p><span className="font-medium text-gray-900">Name:</span> {item.product.name}</p>
                  <p><span className="text-green-600 font-medium">Price:</span> ฿{PriceWithDiscount(item.product.price, item.product.discount).toFixed(2)}</p>
                  <p><span className="text-gray-700">Quantity:</span> {item.quantity}</p>
                  <p><span className="text-red-600 font-medium">Total:</span> ฿{(PriceWithDiscount(item.product.price, item.product.discount) * item.quantity).toFixed(2)}</p>
                </div>
              </div>
            ))}
          </div>
