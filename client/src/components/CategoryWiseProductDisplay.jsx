...
import CardLoding from "./CardLoding"
...
const CategoryWiseProductDisplay = ({ id, name }) => {
  ...
  const fetchCategoryWiseProduct = async () => {
    try {
      ...
      if (responseData.success) {
        ...
        // console.log('res111',responseData.data)
  ...
  const handleRedirectProductListPage = () => {
    ...
    const subCategory = subCategoryData.find((sub) => {
      // ตรวจสอบว่า categoryRelations เป็นอาร์เรย์ก่อน
      if (!sub.categoryRelations || !Array.isArray(sub.categoryRelations)) {
        console.error("sub.categoryRelations is undefined or not an array", sub);
        return false;
      }
      // ค้นหา category ภายใน categoryRelations
      return sub.categoryRelations.some((relation) => relation.category.id === id);
    });
